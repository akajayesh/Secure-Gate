...
REGISTER_FILE = 'registered_left.json'
...
with mp_hands.Hands(
    ...
    if registered is not None:
        registered = np.array(registered)  # Align registered landmarks for better matchin
# ... existing code ...
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
        ...
        if results.multi_hand_landmarks and is_real_left_hand(results):
            ...
            if registered is not None and match_landmarks(hand_landmarks, registered):
                if not unlocked:
                    print("Your Left Palm is Registered")
                    unlocked = True
            else:
                unlocked = False
        else:
        ...
        if key == 27:
            break
        elif key == ord('r') and results.multi_hand_landmarks and is_real_left_hand(results):
